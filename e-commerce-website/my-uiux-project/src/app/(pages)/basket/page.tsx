...
//     try {
//       const metadata = {
//         orderNumber: isClient ? crypto.randomUUID() : "",
...
import { createCheckoutSession, GroupedBasketItem, Metadata } from "../../../../actions/createCheckoutSession";
import { CheckoutData } from "@/type/types";
...
const BasketPage = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (!groupItems || groupItems.length === 0) {
      console.error("Basket is empty. Cannot proceed to checkout.");
      return;
    }
    ...
    try {
      const metadata: Metadata = {
        ...
        clerkUserId: user?.id ?? "Unknown", // Ensure user ID is not undefined
      };
      ...
      const lineItems = groupItems.map((item) => {
        ...
        if (!price) {
          console.error("Item price is missing for", item.product.name);
          return null; // Return null for invalid price
        }
  
        return {
          product: item.product, // Include the product object
          price_data: {
            currency: "usd",
            product_data: {
      ...
      const CheckoutUrl = await createCheckoutSession(lineItems as GroupedBasketItem[], metadata); // Type assertion here
